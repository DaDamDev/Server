-- TODO: update visuals to fit new HUD

if SERVER then
	util.AddNetworkString("sbv_propinfo")
	...
	net.Receive("sbv_propinfo", function(len, ply)
	...
	timer.Simple(0, function()
		local pi = CreateClientConVar("sbv_pi", "1", true, false, "This toggle if propinfo will be displayed on screen.")
		local pia = CreateClientConVar("sbv_pia", "1", true, false, "This toggle if propinfo will contain advanced information.")
		...
		-----Constraint stuff-----
		net.Receive("sbv_propinfo", function()
		...
		timer.Create("sbv_propinfo", 1, 0, function()
			if validEnt then
		...
		surface.CreateFont("sbv_propinfo", {
		  font = "Trebuchet24",
		  size = 15,
		  weight = 1000
		})
		
		local font = "sbv_propinfo"
		
		hook.Add("HUDPaint", "sbv_propinfo", function()
			if pi:GetBool() then
