...
local function open()
	if not base then
		...
		skin:SetExpanded(0)
		skin:SetLabel("UI Skin (" .. dadam.skin .. ")")
		
		local skinList = vgui.Create("DPanelList", aimbotFrame)
		...
		function skinWhite:DoClick() dadam.skin = "default" dadam.saveSettings() skin:SetLabel("UI Skin (" .. dadam.skin .. ")") end
		
		local skinDark = vgui.Create("DButton")
		skinDark:SetText("Dark")
		skinList:AddItem(skinDark)
		function skinDark:DoClick() dadam.skin = "CISKIN" dadam.saveSettings()  skin:SetLabel("UI Skin (" .. dadam.skin .. ")") end
